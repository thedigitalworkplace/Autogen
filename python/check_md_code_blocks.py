...
def check_code_blocks(markdown_file_paths: List[str]) -> None:
    ...
    for markdown_file_path in markdown_file_paths:
        ...
        for code_block, line_no in code_blocks:
            ...
            logger.info(
                "Checking a code block in %s...", markdown_file_path_with_line_no
            )

            # Skip blocks that don't import autogen_agentchat, autogen_core, or autogen_ext
            if all(
                all(
                    import_code not in code_block
                    for import_code in [f"import {module}", f"from {module}"]
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check code blocks in Markdown files for syntax errors."
    )
