...
async def test_register_receives_publish_cascade_single_worker() -> None:
    ...
    for _ in range(num_initial_messages):
        await runtime.publish_message(
            CascadingMessageType(round=1), topic_id=DefaultTopicId()
        )
...
async def test_register_receives_publish_cascade_multiple_workers() -> None:
    ...
    publisher.add_message_serializer(
        try_get_known_serializers_for_type(CascadingMessageType)
    )
    publisher.start()
    for _ in range(num_initial_messages):
    ...
    # Check that each agent received the correct number of messages.
    for i in range(num_agents):
        agent = await workers[i].try_get_underlying_agent_instance(
            AgentId(f"name{i}", "default"), CascadingAgent
        )
        assert agent.num_calls == total_num_calls_expected
...
async def test_default_subscription() -> None:
    ...
    await LoopbackAgentWithDefaultSubscription.register(
        worker, "name", lambda: LoopbackAgentWithDefaultSubscription()
    )
...
async def test_default_subscription_other_source() -> None:
    ...
    await LoopbackAgentWithDefaultSubscription.register(
        runtime, "name", lambda: LoopbackAgentWithDefaultSubscription()
    )
...
async def test_type_subscription() -> None:
    ...
    await LoopbackAgentWithSubscription.register(
        worker, "name", lambda: LoopbackAgentWithSubscription()
    )

    await publisher.publish_message(
        MessageType(), topic_id=TopicId(type="Other", source="default")
    )
...
async def test_disconnected_agent() -> None:
    ...
    try:
        ...
        await worker1.publish_message(
            ContentMessage(content="Hello!"), DefaultTopicId()
        )
        ...
        assert len(set(recipients2)) == len(
            recipients2
...
async def test_proto_payloads() -> None:
    ...
    publisher_runtime.add_message_serializer(
        try_get_known_serializers_for_type(ProtoMessage)
    )
    publisher_runtime.start()

    await ProtoReceivingAgent.register(receiver_runtime, "name", ProtoReceivingAgent)

    await publisher_runtime.publish_message(
        ProtoMessage(message="Hello!"), topic_id=DefaultTopicId()
    )
...
async def test_grpc_max_message_size() -> None:
    ...
    worker2 = GrpcWorkerAgentRuntime(host_address=host_address)
    worker3 = GrpcWorkerAgentRuntime(
        host_address=host_address, extra_grpc_config=extra_grpc_config
    )
    ...
    try:
        ...
        assert (
            agent_instance_3.num_calls == 2
        )  # Worker 3 will receive the big message as has increased message length
