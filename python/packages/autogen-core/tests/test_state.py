...
async def test_agent_can_save_state() -> None:
    ...
    agent1: StatefulAgent = await runtime.try_get_underlying_agent_instance(
        agent1_id, type=StatefulAgent
    )
...
async def test_runtime_can_save_state() -> None:
    ...
    agent2: StatefulAgent = await runtime2.try_get_underlying_agent_instance(
        agent2_id, type=StatefulAgent
    )
