...
async def test_round_robin_group_chat(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            ...
            and result.stop_reason == "Text 'TERMINATE' mentioned"
        )
...
async def test_round_robin_group_chat_state() -> None:
    ...
    team2 = RoundRobinGroupChat(
        participants=[agent3, agent4], termination_condition=termination
    )
    await team2.load_state(state)
    state2 = await team2.save_state()
    assert state == state2

    agent1_model_ctx_messages = (
        await agent1._model_context.get_messages()
    )  # pyright: ignore
    agent2_model_ctx_messages = (
        await agent2._model_context.get_messages()
    )  # pyright: ignore
    agent3_model_ctx_messages = (
        await agent3._model_context.get_messages()
    )  # pyright: ignore
    agent4_model_ctx_messages = (
        await agent4._model_context.get_messages()
    )  # pyright: ignore
    assert agent3_model_ctx_messages == agent1_model_ctx_messages
    assert agent4_model_ctx_messages == agent2_model_ctx_messages
    manager_1 = (
        await team1._runtime.try_get_underlying_agent_instance(  # pyright: ignore
...
@pytest.mark.asyncio
async def test_round_robin_group_chat_with_tools(
    ...
    termination = TextMentionTermination("TERMINATE")
    team = RoundRobinGroupChat(
        participants=[tool_use_agent, echo_agent], termination_condition=termination
    )
    result = await team.run(
        task="Write a program that prints 'Hello, world!'",
    )
    ...
    assert isinstance(
        result.messages[3], ToolCallSummaryMessage
    ...
    index = 0
    await team.reset()
    result2 = await Console(
        team.run_stream(task="Write a program that prints 'Hello, world!'")
    )
...
async def test_round_robin_group_chat_max_turn() -> None:
    ...
    agent_4 = _EchoAgent("agent_4", description="echo agent 4")
    team = RoundRobinGroupChat(
        participants=[agent_1, agent_2, agent_3, agent_4], max_turns=3
    )
    result = await team.run(
        task="Write a program that prints 'Hello, world!'",
    )
...
async def test_round_robin_group_chat_cancellation() -> None:
    ...
    team = RoundRobinGroupChat(
        participants=[agent_1, agent_2, agent_3, agent_4], max_turns=1000
    )
    ...
    total_messages = (
        agent_1.total_messages
        + agent_2.total_messages
        + agent_3.total_messages
        + agent_4.total_messages
    )
        ...
        ChatCompletion(
            ...
            choices=[
                Choice(
                    finish_reason="stop",
                    index=0,
                    message=ChatCompletionMessage(content="agent3", role="assistant"),
                )
            ],
            created=0,
            model=model,
        ...
        ChatCompletion(
            ...
            choices=[
                Choice(
                    finish_reason="stop",
                    index=0,
                    message=ChatCompletionMessage(content="agent2", role="assistant"),
                )
            ],
            created=0,
            model=model,
        ...
        ChatCompletion(
            ...
            choices=[
                Choice(
                    finish_reason="stop",
                    index=0,
                    message=ChatCompletionMessage(content="agent1", role="assistant"),
                )
            ],
            created=0,
            model=model,
...
@pytest.mark.asyncio
async def test_selector_group_chat_two_speakers(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    model = "gpt-4o-2024-05-13"
    chat_completions = [
...
@pytest.mark.asyncio
async def test_selector_group_chat_two_speakers_allow_repeated(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    model = "gpt-4o-2024-05-13"
    chat_completions = [
...
@pytest.mark.asyncio
async def test_selector_group_chat_custom_selector(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    model = "gpt-4o-2024-05-13"
    chat_completions = [
...
async def test_swarm_handoff() -> None:
    ...
    termination = MaxMessageTermination(6)
    team = Swarm(
        [second_agent, first_agent, third_agent], termination_condition=termination
    )
    result = await team.run(task="task")
    assert len(result.messages) == 6
    ...
    team2 = Swarm(
        [second_agent2, first_agent2, third_agent2], termination_condition=termination
    )
    await team2.load_state(state)
    state2 = await team2.save_state()
    assert state == state2
...
async def test_swarm_with_handoff_termination() -> None:
    ...
    result = await team.run(
        task=HandoffMessage(
            content="Handoff to first_agent.", target="first_agent", source="user"
        )
    )
    assert len(result.messages) == 4
...
async def test_round_robin_group_chat_with_message_list() -> None:
    ...
    assert (
        result.stop_reason
        == "Maximum number of messages 4 reached, current message count: 4"
    )
    ...
    with pytest.raises(
        ValueError, match="All messages in task list must be valid ChatMessage types"
    ):
