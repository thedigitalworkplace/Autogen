...
class LedgerOrchestrator(BaseOrchestrator):
    ...
    async def _initialize_task(
        self, task: str, cancellation_token: Optional[CancellationToken] = None
    ) -> None:
        ...
        planning_conversation.append(
            UserMessage(
                content=self._get_closed_book_prompt(self._task),
        ...
        planning_conversation.append(
            AssistantMessage(content=self._facts, source=self.metadata["type"])
        )
        ...
        planning_conversation.append(
            UserMessage(
                content=self._get_plan_prompt(self._team_description),
                source=self.metadata["type"],
            )
        )
        response = await self._model_client.create(
    ...
    async def _update_facts_and_plan(
        ...
        planning_conversation.append(
            UserMessage(
                content=self._get_update_facts_prompt(self._task, self._facts),
                source=self.metadata["type"],
            )
        )
        response = await self._model_client.create(
        ...
        planning_conversation.append(
            UserMessage(
                content=self._get_update_plan_prompt(self._team_description),
                source=self.metadata["type"],
            )
        )
        response = await self._model_client.create(
    ...
    async def update_ledger(
        ...
        ledger_user_messages: List[LLMMessage] = [
            UserMessage(content=ledger_prompt, source=self.metadata["type"])
        ]
                ...
                for key in required_keys:
                    if key not in ledger_dict:
                        ledger_user_messages.append(
                            AssistantMessage(content=ledger_str, source="self")
                        )
                        ledger_user_messages.append(
                            UserMessage(
                                content=f"KeyError: '{key}'",
                                source=self.metadata["type"],
                            )
                        )
                        key_error = True
    ...
    async def _prepare_final_answer(
        ...
        final_message = UserMessage(
            content=ORCHESTRATOR_GET_FINAL_ANSWER.format(task=self._task),
            source=self.metadata["type"],
        )
        response = await self._model_client.create(
    ...
    async def _select_next_agent(
        ...
        if len(self._task) == 0:
            await self._initialize_task(
                self._get_message_str(message), cancellation_token
            )
            ...
            await self.publish_message(
                BroadcastMessage(
                    content=UserMessage(
                        content=synthesized_prompt, source=self.metadata["type"]
                    )
                ),
                topic_id=topic_id,
                cancellation_token=cancellation_token,
            ...
            synthesized_message = AssistantMessage(
                content=synthesized_prompt, source=self.metadata["type"]
            )
            ...
            return await self._select_next_agent(
                synthesized_message, cancellation_token
            )
        ...
        stalled = (
            ledger_dict["is_in_loop"]["answer"]
            or not ledger_dict["is_progress_being_made"]["answer"]
        )
        if stalled:
        ...
        for agent in self._agents:
            if (await agent.metadata)["type"] == next_agent_name:
                ...
                user_message = UserMessage(
                    content=instruction, source=self.metadata["type"]
                )
