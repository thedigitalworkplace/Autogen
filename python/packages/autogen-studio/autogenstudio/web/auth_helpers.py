...
def get_user_session(request: Request):
    ...
    email = request.headers.get("X-User-Email")
    if not email:
        raise HTTPException(status_code=401, detail="Unauthorized: Email header is missing")

    # Define the API URL and authentication token for app.officio.work
    officio_api_url = os.getenv("OFFICIO_API_URL", "https://app.officio.work/api/v1/get-user-details")
    api_token = os.getenv("OFFICIO_API_TOKEN")
    if not api_token:
        raise HTTPException(status_code=500, detail="Server misconfiguration: API token is missing")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to connect to app.officio.work: {str(e)}")

    # Parse the response
    user_data = response.json()
    if not user_data.get("status") or "user" not in user_data.get("data", {}):
        raise HTTPException(status_code=404, detail="User not found in app.officio.work")
